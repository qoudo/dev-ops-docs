...
Так же, если ввести в адресной строке браузера localhost, то должна отобразиться приветственная страница Nginx:
...
Можно взять любой проект или одностраничный html. Если у вас полноценный микросервис, то нам нужен build проекта для 
...
В рамках нашей задачи нужно создать SLL сертификат для https соеденений. Для простоты решения используем openssl:
```bash
...
После того как мы успешно настроили конфигурацию перейдем к проверки нашего сервера. Но перед этим его необходимо
...
И увидим svg изображение из нашего сервиса.
...
Можем заметить, что браузер ругается: "адрес не защищен", т.к. у нас не действительный сертификат:
